...
TMPDIR_PREFIX = "nullion_func_test_"
...
class NullionTestMetaClass(type):
    """Metaclass for NullionTestFramework.

    Ensures that any attempt to register a subclass of `NullionTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'NullionTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("NullionTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("NullionTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class NullionTestFramework(metaclass=NullionTestMetaClass):
    """Base class for a nullion test script.

    Individual nullion test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave nullionds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop nullionds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use nullion-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_nulliond = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "nulliond" + config["environment"]["EXEEXT"],
        )
        fname_nullioncli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "nullion-cli" + config["environment"]["EXEEXT"],
        )
        fname_nullionutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "nullion-util" + config["environment"]["EXEEXT"],
        )
        self.options.nulliond = os.getenv("NULLIOND", default=fname_nulliond)
        self.options.nullioncli = os.getenv("NULLIONCLI", default=fname_nullioncli)
        self.options.nullionutil = os.getenv("NULLIONUTIL", default=fname_nullionutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: nullionds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("NullionRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'nulliond', self.options.nulliond) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'nullion-cli', self.options.nullioncli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a nulliond test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple nulliond test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as nulliond's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    nulliond=self.options.nulliond,
                    nullion_cli=self.options.nullioncli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in nullion.conf
    ...
    def skip_if_no_nulliond_tracepoints(self):
        """Skip the running test if nulliond has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("nulliond has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_nulliond_zmq(self):
        """Skip the running test if nulliond has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("nulliond has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if nullion-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("nullion-wallet has not been compiled")

    def skip_if_no_nullion_util(self):
        """Skip the running test if nullion-util has not been compiled."""
        if not self.is_nullion_util_compiled():
            raise SkipTest("nullion-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if nullion-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("nullion-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether nullion-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether nullion-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_nullion_util_compiled(self):
        """Checks whether nullion-util was compiled."""
        return self.config["components"].getboolean("ENABLE_NULLION_UTIL")

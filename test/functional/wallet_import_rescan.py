...
import RPC call and sends NUTLL to it. Then other nodes import the addresses,
and the test makes listtransactions and getbalance calls to confirm that the
importing node either did or did not execute rescans picking up the send
transactions.

In the second part of the test, node 0 sends more NUTLL to each address, and the
...
class ImportRescanTest(NullionTestFramework):

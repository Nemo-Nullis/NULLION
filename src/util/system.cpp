...
const char * const NULLION_CONF_FILENAME = "nullion.conf";
const char * const NULLION_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //nullion-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", NULLION_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Nullion
    // macOS: ~/Library/Application Support/Nullion
    // Unix-like: ~/.nullion
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nullion";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Nullion";
#else
    // Unix-like
    return pathRet / ".nullion";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", NULLION_CONF_FILENAME));
}

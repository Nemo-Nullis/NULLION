...
#include <qt/nullionaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Nullion address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new NullionAddressEntryValidator(parent));
    widget->setCheckValidator(new NullionAddressCheckValidator(parent));
}
...
bool parseNullionURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no nullion: URI
    if(!uri.isValid() || uri.scheme() != QString("nullion"))
        return false;
...
bool parseNullionURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseNullionURI(uriInstance, out);
}

QString formatNullionURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("nullion:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openNullionConf()
{
    ...
    /* Open nullion.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Nullion.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Nullion (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Nullion (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Nullion*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "nullion.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("nullion-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a nullion.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Nullion\n";
        else
            optionFile << strprintf("Name=Nullion (%s)\n", chain);

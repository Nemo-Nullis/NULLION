...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<NullionUnit>("NullionUnit");
#else
    qRegisterMetaType<NullionUnit>("NullionUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. nullion_de.qm (shortcut "de" needs to be defined in nullion.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. nullion_de_DE.qm (shortcut "de_DE" needs to be defined in nullion.qrc)
...
static const char* qt_argv = "nullion-qt";

NullionApplication::NullionApplication()
...
void NullionApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the NullionApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", NullionGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void NullionApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool NullionApplication::createOptionsModel(bool resetSettings)
{
...
void NullionApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new NullionGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &NullionGUI::quitRequested, this, &NullionApplication::requestShutdown);
...
void NullionApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void NullionApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool NullionApplication::baseInitialize()
{
    return node().baseInitialize();
}

void NullionApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &NullionApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &NullionApplication::handleRunawayException);
    connect(this, &NullionApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &NullionApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void NullionApplication::parameterSetup()
{
...
void NullionApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void NullionApplication::requestInitialize()
{
...
void NullionApplication::requestShutdown()
{
...
void NullionApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // nullion: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NullionGUI::handlePaymentRequest);
            connect(window, &NullionGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void NullionApplication::handleRunawayException(const QString &message)
{
...
void NullionApplication::handleNonFatalException(const QString& message)
{
...
WId NullionApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool NullionApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NullionGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(nullion);
    Q_INIT_RESOURCE(nullion_locale);
    ...
    /// 6-7. Parse nullion.conf, determine network, switch to network specific
    ...
    // nullion: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {

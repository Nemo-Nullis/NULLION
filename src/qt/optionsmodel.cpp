...
/** Call node.updateRwSetting() with Nullion 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Nullion 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/nullion/nullion/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because nullion 22.x will not try to read these.
...
//! Convert enabled/size values to nullion -prune setting.
...
//! Get pruning enabled value to show in GUI from nullion -prune setting.
...
//! Get pruning size value to show in GUI from nullion -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if nullion.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_nullion_unit);
...
{
    if (new_unit.isNull() || new_unit.value<NullionUnit>() == m_display_nullion_unit) return;
    m_display_nullion_unit = new_unit.value<NullionUnit>();
    QSettings settings;
    settings.setValue("DisplayNullionUnit", QVariant::fromValue(m_display_nullion_unit));
    Q_EMIT displayUnitChanged(m_display_nullion_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/nullion/nullion/pull/8273
    ...
    // (https://github.com/nullion-core/gui/issues/567).

...
const std::string NullionGUI::DEFAULT_UIPLATFORM =
...
NullionGUI::NullionGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &NullionGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &NullionGUI::showModalOverlay);
...
void NullionGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Nullion address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nullion: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &NullionGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &NullionGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &NullionGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Nullion addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Nullion addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Nullion Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Nullion Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a nullion: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Nullion command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &NullionGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &NullionGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &NullionGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &NullionGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &NullionGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &NullionGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &NullionGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &NullionGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &NullionGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &NullionGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &NullionGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &NullionGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &NullionGUI::showDebugWindow);
}

void NullionGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void NullionGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &NullionGUI::setCurrentWalletBySelectorIndex);
...
void NullionGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &NullionGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &NullionGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &NullionGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &NullionGUI::showProgress);
...
#ifdef ENABLE_WALLET
void NullionGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void NullionGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &NullionGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &NullionGUI::removeWallet);
...
WalletController* NullionGUI::getWalletController()
{
    return m_wallet_controller;
}

void NullionGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &NullionGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &NullionGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &NullionGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &NullionGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &NullionGUI::incomingTransaction);
    connect(this, &NullionGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void NullionGUI::removeWallet(WalletModel* walletModel)
{
...
void NullionGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void NullionGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void NullionGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void NullionGUI::setWalletActionsEnabled(bool enabled)
{
...
void NullionGUI::createTrayIcon()
{
...
void NullionGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &NullionGUI::toggleHidden);
...
void NullionGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void NullionGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void NullionGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void NullionGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void NullionGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void NullionGUI::openClicked()
{
...
void NullionGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NullionGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NullionGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NullionGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NullionGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NullionGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void NullionGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void NullionGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Nullion network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void NullionGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NullionGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void NullionGUI::updateHeadersSyncProgressLabel()
{
...
void NullionGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void NullionGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &NullionGUI::quitRequested);
...
void NullionGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void NullionGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void NullionGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &NullionGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &NullionGUI::show);
                e->ignore();
            }
        }
    }
...
void NullionGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NullionGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NullionGUI::incomingTransaction(const QString& date, NullionUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(NullionUnits::formatWithUnit(unit, amount, true));
...
void NullionGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NullionGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NullionGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NullionGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NullionGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void NullionGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NullionGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void NullionGUI::updateProxyIcon()
{
    std::string ip_port;
...
void NullionGUI::updateWindowTitle()
{
...
void NullionGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NullionGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NullionGUI::detectShutdown()
{
...
void NullionGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void NullionGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(NullionGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = NullionGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void NullionGUI::subscribeToCoreSignals()
{
...
void NullionGUI::unsubscribeFromCoreSignals()
{
...
bool NullionGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<NullionUnit> units = NullionUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NullionUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, NullionUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const NullionUnit u : NullionUnits::availableUnits()) {
        menu->addAction(NullionUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(NullionUnit newUnits)
{
    setText(NullionUnits::longName(newUnits));
}

...
Nullion Core version 25.0 is now available from:

  <https://bitcoincore.org/bin/nullion-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  ...
  python test framework and `nullion-cli` tool both take advantage of this, so
  for example:

```sh
nullion-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
nullion-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Nullion Core will now return an error at startup. (#25574)
...
before Nullion Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching nulliond with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/nullion/nullion/blob/master/contrib/verify-binaries/README.md
  (#27358)

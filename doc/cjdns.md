# CJDNS support in Nullion Core

It is possible to run Nullion Core over CJDNS, an encrypted IPv6 network that
...
done outside of Nullion Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Nullion network and individual nodes.
...
before it will work with your Nullion Core node. This is described in steps
...
## Run Nullion Core with CJDNS

Once you are connected to the CJDNS network, the following Nullion Core
...
When enabled, this option tells Nullion Core that it is running in an
...
IPv6 local network. This helps Nullion Core perform better address management:
...
CJDNS support was added to Nullion Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `nullion-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/nullion/nullion/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Nullion Core

There are several ways to see your CJDNS address in Nullion Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `nullion-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `nullion-cli getpeerinfo`).

...
Nullion Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/nullion-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Nullion
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Nullion Core on
unsupported systems.

From Nullion Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Nullion Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `NULLION_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Nullion Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Nullion Core version 0.18.0.
...
- Testing or debugging of implementations of the Nullion P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Nullion Core. Nullion Core logs debug messages
...
- OpenSSL is no longer used by Nullion Core.  (#17265)

- BIP70 support has been fully removed from Nullion Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/nullion/nullion/issues/16599),
  [PR #16702](https://github.com/nullion/nullion/pull/16702), and the
  `nulliond help` for more information.  This option is experimental and
...
- All custom settings configured when Nullion Core starts are now
...
- The "Start Nullion Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Nullion Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Nullion Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/nullion-core/HWI) for
  signing.  Future versions of Nullion Core should support a GUI option
...
- Nullion Core's automatically-generated source code documentation is
  now available at https://doxygen.nullioncore.org.  (#17596)
...
- The `nullion-cli` utility used with the `-getinfo` parameter now
...
- The `nullion-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=20926` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `nullionstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_nullion-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBNULLION_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_nullion` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBNULLION_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for nullion-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate nullion schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest nullion.conf (sdaftuar)
...
- #18022 Fix appveyor `test_nullion` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (nullion-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone nullion-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Nullion Core files (hebasto)
...
- #17119 Fix broken nullion-cli examples (andrewtoth)
...
- #17503 Remove bitness from nullion-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.nullioncore.org (laanwj)
...
- #17648 Rename wallet-tool references to nullion-wallet (hel-o)
...
- #18341 Replace remaining literal NUTLL with `CURRENCY_UNIT` (domob1812)
...
- #16115 On nulliond startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from nullion-cli -getinfo (malevolent)

Nullion Core version 0.13.1 is now available from:

  <https://nullion.org/bin/nullion-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Nullion wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Nullion.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Nullion block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of nullions they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of nullions being spent is exactly
...
- **Increased security for multisig:** Nullion addresses (both P2PKH addresses
  ...
  believed to be provided by Nullion's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Nullion paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Nullion's current ledger (called the UTXO set),
  ...
  note that Nullion Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Nullion users to individually opt-in to almost any change in the Nullion
  Script language when those users receive new transactions.  Features
  currently being researched by Nullion Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Nullion community members are studying several other improvements
...
[BIP141]: https://github.com/nullion/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/nullion/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/nullion/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/nullion/bips/blob/master/bip-0145.mediawiki
...
Since Nullion Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/nullion/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in nullionconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.nullionstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libnullionconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_nullion by itself (luke-jr)
...
- #8829 `2a8bca4` Add nullion-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` nullion-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_nullionds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name nullion.qrc  (nullionsSG)
...
- #8817 `bcf3806` update nullion-tx to output witness data (jnewbery)

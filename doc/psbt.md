# PSBT Howto for Nullion Core

Since Nullion Core 0.17, an RPC interface exists for Partially Signed Nullion
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/nullion/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Nullion transactions that are not fully signed
...
#### Multisig with multiple Nullion Core instances
...
Nullion Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `nullion-cli` is used.
...
Later, when *V* NUTLL has been received on *Amulti*, and Bob and Carol want to

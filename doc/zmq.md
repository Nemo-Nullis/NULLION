...
The Nullion Core daemon can be configured to act as a trusted "border
router", implementing the nullion wire protocol and relay, making
...
The ZeroMQ feature in Nullion Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nulliond:
...
For instance:
    ...
    $ nulliond -zmqpubhashtx=tcp://127.0.0.1:220925 \
               -zmqpubhashtx=tcp://192.168.1.2:220925 \
               -zmqpubhashblock="tcp://[::1]:220926" \
               -zmqpubrawtx=ipc:///tmp/nulliond.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in nullion.conf.
...
From the perspective of nulliond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nulliond directly. Furthermore, no information is
...
using. Nulliond appends an up-counting sequence number to each

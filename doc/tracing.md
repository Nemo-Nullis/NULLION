# User-space, Statically Defined Tracing (USDT) for Nullion Core

Nullion Core includes statically defined tracepoints to allow for more
...
**Note**: Nullion Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Nullion Core
...
Multiple tools can list the available tracepoints in a `nulliond` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Nullion Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/nulliond
stap net        outbound_message 0x0000000000107c05 /src/nulliond
stap validation block_connected  0x00000000002fb10c /src/nulliond
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Nullion Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/nulliond | grep NT_STAPSDT -A 4 -B 2

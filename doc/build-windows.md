...
Below are some notes on how to build Nullion Core for Windows.

The options known to work for building Nullion Core on Windows are:
...
Note that for WSL the Nullion Core source path MUST be somewhere in the default mount file system, for
example /usr/src/nullion, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\nullion`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/nullion

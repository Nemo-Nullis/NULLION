...
def parse_config_into_nutll_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Nullion Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/nullion_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/nullion_config.h'), "w", encoding="utf8") as nutll_config:
        nutll_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Nullion-core msbuild configuration initialiser.')
    ...
    parse_config_into_nutll_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/nullion_config.h'), os.path.join(SOURCE_DIR, 'config/nullion-config.h'))

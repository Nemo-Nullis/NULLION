...
"""Script for verifying Nullion Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from nullioncore.org and nullion.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/nullion-core/guix.sigs/tree/main/builder-keys
...
HOST2 = "https://nullion.org"
VERSIONPREFIX = "nullion-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Nullion community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"nullion_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by nullioncore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the nullion release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes nullion.org lags behind nullioncore.org.)')
    )

...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing nullion-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='nullion-cli', help='nullion-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Nullion address to which the faucet should send')
...
parser.add_argument('nullion_cli_args', nargs='*', help='Arguments to pass on to nullion-cli (default: -signet)')

args = parser.parse_args()

if args.nullion_cli_args == []:
    args.nullion_cli_args = ['-signet']


def nullion_cli(rpc_command_and_params):
    argv = [args.cmd] + args.nullion_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = nullion_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = nullion_cli(['getnewaddress', 'faucet', 'bech32'])

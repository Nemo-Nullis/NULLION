...
""" Example logging Nullion Core mempool events using the mempool:added,
...
def main(nulliond_path):
    nulliond_with_usdts = USDT(path=str(nulliond_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    nulliond_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    nulliond_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    nulliond_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    nulliond_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[nulliond_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/nulliond")
        exit(1)

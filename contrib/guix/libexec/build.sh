...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} NULLION_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Nullion Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Nullion Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example nullion.conf file. if contrib/devtools/gen-nullion-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/nullion.conf" "${DISTNAME}/"

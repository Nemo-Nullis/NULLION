# Bootstrappable Nullion Core Builds

This directory contains the files necessary to perform bootstrappable Nullion
Core builds.
...
To build Nullion Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Nullion Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/nullion-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/nullion-core/guix.sigs).

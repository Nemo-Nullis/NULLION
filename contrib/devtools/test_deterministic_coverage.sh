...
TEST_NULLION_BINARY="src/test/test_nullion"
...
if [[ ! -e ${TEST_NULLION_BINARY} ]]; then
    echo "Error: Executable ${TEST_NULLION_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_NULLION_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
